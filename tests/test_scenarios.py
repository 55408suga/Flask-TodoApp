def test_user_flow_happy_path(client):
    ...
    # 1. ユーザー登録
    res = client.post(
        "/api/register", json={"username": "hero_tanaka", "password": "secure_password"}
    )
    assert res.status_code == 201

    # 2. ログイン
    res = client.post(
        "/api/login", json={"username": "hero_tanaka", "password": "secure_password"}
    )
    ...
    res = client.post("/api/todos", json={"name": "世界を救う", "is_done": False})
    assert res.status_code == 204
